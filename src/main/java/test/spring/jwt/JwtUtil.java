...
public class JwtUtil implements Serializable {
	...
	private Date generateExpirationDate() {
		return new Date(System.currentTimeMillis() + expirationPeriod * 60000);
	}

	private Boolean isTokenExpired(String token) {
		final Date expiration = getExpirationDateFromToken(token);
		LOGGER.info("Expiration Datetime: "+expiration.toString());
	...
	public String generateToken(UserDetail jwtUser) {
	...
	public Boolean validateToken(String token, UserDetail jwtUser) {
//		final Date created = getCreatedDateFromToken(token);
		// final Date expiration = getExpirationDateFromToken(token);
		return (jwtUser != null && !isTokenExpired(token));
	}
